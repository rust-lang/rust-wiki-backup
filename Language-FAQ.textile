...
** Structural:http://en.wikipedia.org/wiki/Structural_type_system (rather than http://en.wikipedia.org/wiki/Nominative_type_system:nominal) object types; no class-based hierarchy.
...
The Rust compiler, 30,000 lines at the time of writing, is written in Rust. See the <code>src/comp/</code> directory. For some smaller samples, check out the benchmark programs in <code>src/test/bench</code>.
...
h3. Why are <code>log</code> and <code>log_err</code> statements rather than library functions?
