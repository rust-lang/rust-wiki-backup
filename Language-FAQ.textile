...
* An box pointer points into a reference-counted heap allocation.
...
h3. Why can't state objects have destructors? Why only immutable ones?

Because state-ness places an object's destruction sequence under control of the garbage collection system, rather than the reference counting system, and this makes it potentially unclear which order to destroy objects in (the garbage graph may be cyclic, in which case there is no meaning to "top-down".)

If you want a state object to have something destructor-like, the easiest thing to do is split some minimal immutable parts of it off into a separate sub-object and give that sub-object a destructor. This works because while immutable memory cannot contain mutable memory, the opposite is just fine: the immutable subgraph hanging off the state object has a well-defined, top-down destruction order.
...
That's true. For a while we played with having the type system differentiate "cyclic" from "state" as different effects, but it seemed to be a confusing distinction (and not always possible to determine, for example given an object or bound function, the contents of its binding is abstract). We decided to remove the distinction. Besides, as the second paragraph above indicates, it's not so hard to work around by splitting up stateful and immutable pieces of an object.