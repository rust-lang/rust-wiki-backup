...
h3. Why are lexemes limited to ASCII outside comments, strings and characters? Why not use the most general Unicode character class for every lexeme?
...
h3. Why are strings UTF8 by default? Why not UCS2 or UCS4?

The <code>str</code> type is UTF8 because we observe more text in the wild in this encoding -- particularly in network transmissions, which are endian-agnostic -- and we think it's best that the default treatment of I/O not involve having to recode codepoints in each direction.
...
h3. Why is <code>log</code> a statement rather than a library function?
...
h3. Why are channels, tasks, ports, strings, vectors etc. built-in types rather than (say) special kinds of obj?
...
They start very small (a few hundred bytes) and expand dynamically by doubling reallocation, like a vector. Stack pointers held inside a task's stack are rewritten when a stack grows. Stack pointers can never escape to the heap in Rust -- by design -- so the heap does not need to be scanned during stack growth.

h3. What is the difference between an alias and an exterior pointer?

* An exterior pointer points into a reference-counted heap allocation.
...
* Think of aliases as "pass by reference". They are no for holding long-term, stable references, just for references passing between functions.
...
h3. Why can't mutable objects have destructors? Why only immutable ones?

Because mutability places an object's destruction sequence under control of the garbage collection system, rather than the reference counting system, and this makes it potentially unclear which order to destroy objects in (the garbage graph may be cyclic, in which case there is no meaning to "top-down".)

If you want a mutable object to have something destructor-like, the easiest thing to do is split the immutable part of it off into a separate sub-object and give that sub-object a destructor. This works because while immutable memory cannot contain mutable memory, the opposite is just fine: the immutable subgraph hanging off the mutable one has a well-defined, top-down destruction order.

h3. Isn't that a bit over-simplistic? Not every mutable value is cyclic! Some may be structurally acyclic, even if they contain mutable bits.

That's true. For a while we played with having the type system differentiate "cyclic" from "state" as different effects, but it seemed to be a confusing distinction (and not always possible to determine, for example given an object or bound function, the contents of its binding is abstract). We decided to remove the distinction. Besides, as the second paragraph above indicates, it's not so hard to work around by splitting up mutable and immutable pieces of an object.