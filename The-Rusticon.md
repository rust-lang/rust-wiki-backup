...
<a name="reference" /> reference | Also known as a "borrowed pointer". It references an object without taking ownership of it. Has an associated lifetime, to assert that it is always valid.
...
unbox | To get the value out of a box
...
`~[T]` | A vector.
`[T, ..n]` | An *array* (sometimes also known as a "fixed length vector") of length `n`.
