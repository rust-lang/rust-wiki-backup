...
* Commit your change but do _not_ push to `master`; instead push to `snap-stage3`. You do this using the `local:remote` convention in git, pushing _your_ `master` branch to _graydon's_ `snap-stage3` branch. That is, you run:
```
$ git push master:snap-stage3
```
