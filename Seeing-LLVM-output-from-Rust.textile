...
@--save-temps@ and then use @llvm-dis@ to look at the resulting LLVM bitcode file.

h2. Long answer

If the test you want to run is in @test/run-pass/foobar.rs@, the invocation (as described in [[The Rust test suite]] page) is

bc. make check TESTNAME=test/run-pass/foobar.rs
...
bc. executing stage2/bin/rustc ../rust/src/test/run-pass/hello.rs -o test/run-pass/hello.stage2 -O --save-temps
