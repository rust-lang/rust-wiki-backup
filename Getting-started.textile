...
* LLVM SVN revision 110060, built for x86 (not x64 if you have one!) and @--enable-shared@.

h4. Debian-based Linux distributions

@sudo apt-get install ocamlc ocaml-native-compilers@
...
Currently (hopefully this will change soon), the Linux and OS X libs *must* be called libLLVM-2.8svn.so and libLLVM-2.8svn.dylib for rustc to find them.
