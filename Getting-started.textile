...
* Python 3.1 ("download link":http://www.python.org/download/) (Python 2.6.2 has also been used successfully)
* GNU make 3.81
* git 1.7
...
* LLVM SVN, built for x86 (not x64 if you have one!).  See the directions below for how to build LLVM.

Rust regularly incorporates new features and fixes from LLVM, so it's best to just assume that the very latest LLVM SVN revision is required. If you find that Rust doesn't build with your version of LLVM, then ask on IRC and somebody will tell you which revision is working for them.
...
Rust requires a 3.0svn build of LLVM. Follow the instructions on "Clang's Getting Started Page":http://clang.llvm.org/get_started.html to check out LLVM (and Clang, if you like).  *Don't follow the build instructions on that page -- see below.*

Because Rust is building against the LLVM's trunk, not every revision will work correctly. Here are revisions that are known to work for each platform:
* Linux: 131752
* Windows: 
* OS X: 131752

You will also need to check out compiler-rt *revision 131034* (later revisions are broken):
