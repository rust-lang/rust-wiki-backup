...
@sudo apt-get install python3 make git g++ curl valgrind texlive@
...
If you install msysgit, you will want to put the git binary path *after* the mingw path. So add a line like:
```
...
Rust requires a 3.0svn build of LLVM. Follow the instructions on "Clang's Getting Started page":http://clang.llvm.org/get_started.html to check out LLVM (and Clang, if you like), but don't follow the build instructions on that page -- see below.*
