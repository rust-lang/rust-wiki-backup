...
* LLVM SVN revision 127624 or newer (**not** any stable release older than March 11, 2011), built for x86 (not x64 if you have one!) and @--enable-shared@.
...
```sh
$ CXX='g++ -m32' CC='gcc -m32' CFLAGS=-m32 CXXFLAGS=-m32 LDFLAGS=-m32 ./configure --enable-shared --disable-bindings --{build,host,target}=i686-apple-darwin --enable-targets=x86,x86_64,cbe
$ make
$ make install
```
...
On 64-bit linux, use:
```sh
$ CXX='g++ -m32' CC='gcc -m32' CFLAGS=-m32 CXXFLAGS=-m32 LDFLAGS=-m32 ./configure --enable-shared --disable-bindings --{build,host,target}=i686-unknown-linux-gnu --enable-targets=x86,x86_64,cbe
$ make
$ make install
```

Currently (hopefully this will change soon), the Linux, OS X and win32 libs *must* be called libLLVM-3.0svn.so, libLLVM-3.0svn.dylib and LLVM-3.0svn.dll, respectively, for rustc to find them. You will need to symlink or copy the resulting library to the "-3.0svn" name if any other version is produced from your build.
...
*Note:* On Linux or OS X, if you have valgrind installed, the tests will run slowly because they are running under valgrind. If you define NO_VALGRIND=1 in your build environment, you can see the tests running at "full" speed (though, of course, the bootstrap compiler generates terrible code).
...
* In addition to the @LD_LIBRARY_PATH@ environment variable, which should be set to something like @$HOME/rust/src:/usr/local/lib@, you'll need to export the @DYLD_LIBRARY_PATH@ environment variable, which should have the same value as @LD_LIBRARY_PATH@.  Use something like @export DYLD_LIBRARY_PATH=$LD_LIBRARY_PATH@.
