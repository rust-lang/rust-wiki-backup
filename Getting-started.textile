...
Rust requires a 3.0svn build of LLVM *at revision 138708*. Follow the instructions on "Clang's Getting Started page":http://clang.llvm.org/get_started.html to check out LLVM (and Clang, if you like), but don't follow the build instructions on that page -- see below.

```sh
svn co -r138708 http://llvm.org/svn/llvm-project/llvm/trunk llvm
```
