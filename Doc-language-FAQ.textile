...
h3. Why isn't `let i: uint = 0` valid Rust?

Rust provides no coercion or overloading for literals so the `0` is _always_ type int. Implicit conversions are a common source of errors in C-like languages, so there is a strong desire to not let such problems creep into the language. That said, this is a common source of frustration, so a limited form of literal overloading may be introduced in the future.
